...
I have included commented code that demonstrates how to add a new page. I would recommend leaving these comments there so you don't forget what modifications you need to make everytime you need to add/remove a page. Here's how to add a new page:
...
* Search _all_ files for "newPage". For each search result, copy and paste the commented line, replace "newPage" with the name of your page. You should find search results in Main.elm and Viewer.elm
...
Add your JavaScript handlers in [src/index.js](src/index.js) (or create a new file if you'd like). Define Elm ports in [src/Ports.elm](src/Ports.elm), and import this file anywhere you need to use a Port.
...
* If you're using create-elm-app, delete [index.html](index.html). Be careful, make sure you delete [index.html at the root](index.html) and not [index.thml in the public/ folder!](public/index.html)
...
Read this section if you don't want to use create-elm-app. Note it is a fairly advanced topic.

If you'd like to use your own dev server instead of create-elm-app, feel free - but beware of issues regarding SPAs. Notably, you'll need a server that supports pushState navigation, 404 redirection, and correctly serving static assets even with nested URLs like `/nest1/nest2/42`. You may also run into issues trying to manage multiple environments and CI/CD.
...
For production, minify and bundle your files using whatever tools you'd like. Be sure that your production server's enviornment variables are set appropriately and support SPAs.
